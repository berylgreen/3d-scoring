            ...
            if item.is_dir():
                ...
                if not item.name.endswith('.xlsx'):
                    try:
                        ...
                        logger.warning(f"  [!] 警告: 无法删除 {item.name}: {e}")
    
    # 清空缩略图缓存
    if THUMBNAIL_CACHE.exists():
        shutil.rmtree(THUMBNAIL_CACHE)
        logger.info(f"  [√] 删除: thumbnail_cache/")
...
def run_batch_grading():
    ...
    if not batch_script.exists():
        logger.error(f"  [x] 错误: 找不到评分脚本 {batch_script}")
        return False
    ...
    if result.returncode == 0:
        ...
        logger.error(f"  [x] 评分程序异常退出 (code: {result.returncode})")
        return False
...
示例:
  ...
  python run_3d_scoring.py --regrade    # 强制重新评分: 清空缓存 + 重新评分
    ...
    logger.info("=" * 50)
    logger.info("大模型评分启动脚本")
    logger.info("=" * 50)
    
    if args.regrade:
        logger.info(f"模式: 强制重新评分")
        clear_cache()
