...
sys.path.insert(0, str(PROJECT_ROOT / "scoring_scripts"))
...
def api_students():
    ...
    for t in targets:
        ...
        if not inds:
            ...
            match = re.match(r'^(\d{4,})[_\-\s—]*([\u4e00-\u9fa5A-Za-z]+)[_\-\s—]*(.*)$', raw_group_name)
            if match:
        ...
        for ind in inds:
            ...
            match = re.match(r'^(\d{4,})[_\-\s—]*([\u4e00-\u9fa5A-Za-z]+)[_\-\s—]*(.*)$', raw_student_name)
            if match:
                parsed_id = match.group(1)
                parsed_name = match.group(2)
                ...
                match2 = re.match(r'^(\d{4,})[_\-\s—]*([\u4e00-\u9fa5A-Za-z]+)[_\-\s—]*(.*)$', raw_group_name)
                if match2:
            ...
            if display_group_name == raw_group_name:
                match3 = re.match(r'^(\d{4,})[_\-\s—]*([\u4e00-\u9fa5A-Za-z]+)[_\-\s—]*(.*)$', raw_group_name)
